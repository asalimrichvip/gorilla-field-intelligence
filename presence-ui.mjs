...
export function presenceView(report,mode,{T,esc,fmt,card,page,pagination,lang}) {
 ...
 </div><section class="panel"><div class="panel-head"><h2>${T('Presence over time','التواجد عبر الزمن')}</h2><span class="tag">Client Code</span></div><p>${T('Presence = Yes to the Gorilla question OR a positive SKU facing count. Cumulative clients keep other filters but start at the first recorded date.','التواجد = نعم لسؤال جوريلا أو وجود صنف بواجهات أكبر من صفر. التراكمي يحتفظ ببقية الفلاتر ويبدأ من أول تاريخ مسجل.')}</p>
 ...
 <div class="table-wrap"><table><thead><tr>${[T('Store','المحل'),'Client Code',T('Latest matching visit','آخر زيارة مطابقة'),T('SKU count','عدد الأصناف'),T('SKU names','أسماء الأصناف'),T('Absent visits in period','مرات الغياب بالفترة')].map(c=>`<th>${c}</th>`).join('')}</tr></thead><tbody>${shown.map(r=>`<tr><td><button class="link-button" data-store="${esc(r._id)}">${esc(r.Client||r._id)}</button></td><td>${esc(r._id)}</td><td>${esc(displayValue('Started At',r['Started At']||r.Date,lang))}</td><td>${r._skuCount===null?T('Incomplete data','بيانات غير مكتملة'):fmt(r._skuCount)}</td><td class="sku-cell">${r._skus.filter(s=>s.quantity>0).map(s=>`<span class="sku-tag" style="--flavor:${flavorColor(s.name)}">${esc(s.name)}</span>`).join('')||'—'}</td><td>${fmt(report.absences.get(r._id)?.length||0)}</td></tr>`).join('')}</tbody></table></div>${outlets.length?pagination(outlets.length):`<p class="empty">${T('No matching stores in the available history.','لا توجد محلات مطابقة في التاريخ المتاح.')}</p>`}
...
export function enrichedPresenceRows(rs,report,lang='en') {
 return rs.map(r=>{
  const original=Object.fromEntries(Object.entries(r).filter(([k])=>!k.startsWith('_')));
