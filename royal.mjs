...
export function visitSeries(rows,by='cycle',selection=[]){
 ...
 const chosen=[...new Set(selection)].filter(k=>buckets.has(k)).slice(0,3);if(!chosen.length)chosen.push(...(period?options.slice(-3):options.slice(0,3)));
 ...
 const count=by==='cycle'?14:by==='month'?31:Math.ceil(span/bin);
 ...
 for(const s of series){let end=last;if(by==='cycle')end=s.start+13;if(by==='month')end=Math.floor(Date.UTC(Number(s.key.slice(0,4)),Number(s.key.slice(5,7)),0)/DAY);
  ...
  for(const r of s.rows){const i=Math.floor((day(r._date)-s.start)/bin);if(i>=0&&i<count)s.values[i]=(s.values[i]??0)+1;}delete s.rows;
 }
 return {options,chosen,series,labels,relative:period,bin};
}
export function rtmShares(rows){const counts=group(rows,'RTM'),total=rows.length;return counts.map(([name,value])=>({name,value,share:total?value/total:0}));}
...
export function mountRoyal({rs,filters,mode,esc,fmt,T,display}){
 ...
 const periods=periodMetrics(rs,mode,filters),stores=latest(rs),charts=[];
 ...
 const drawLines=(el,series,labels,{relative=false,percent=false,unit=T('Visits','الزيارات')}={})=>{
  ...
  series.forEach(s=>{let connected=false;const path=s.values.map((v,i)=>{if(v===null){connected=false;return '';}const p=(connected?'L':'M')+x(i)+' '+y(v);connected=true;return p;}).join(' ');out+=`<path d="${path}" fill="none" stroke="${s.color}" stroke-width="2.3"/>`;s.values.forEach((v,i)=>{if(v!==null)out+=`<circle cx="${x(i)}" cy="${y(v)}" r="3" fill="${s.color}"><title>${esc(s.name||s.key||'')}: ${esc(labels[i])} — ${esc(fmt(v))}</title></circle>`;});});
 ...
 function updateTrend(){model=visitSeries(rs,trendState.by,trendState.selected);trendState.selected=model.chosen;
  seriesControls.innerHTML=[0,1,2].map((slot)=>`<label><i style="background:${palette[slot]}"></i><select data-royal-series="${slot}" aria-label="${esc(T('Comparison ','المقارنة ')+(slot+1))}"><option value="">${T('None','بدون')}</option>${model.options.map(k=>`<option value="${esc(k)}" ${model.chosen[slot]===k?'selected':''} ${model.chosen.includes(k)&&model.chosen[slot]!==k?'disabled':''}>${esc(describe(k))}</option>`).join('')}</select></label>`).join('');
  trend.querySelector('.royal-trend-note').textContent=model.relative?T('Aligned days within each period · active filters apply','أيام متقابلة داخل كل فترة · طبقًا للفلاتر'):T('Same dates · active filters apply','نفس التواريخ · طبقًا للفلاتر');drawTrend();
 }
 ...
 comparison.insertAdjacentHTML('beforeend',`<button class="royal-text-link" data-tab="presencetrends">${T('Details & full history','التفاصيل والتاريخ الكامل')} ↗</button>`);
